...
inline BlockType &operator=(const SparseMatrixBase <OtherDerived> &other) {
    ...
    Index start =
            m_outerStart == 0 ? 0 : matrix.outerIndexPtr()[m_outerStart]; // starting position of the current block
    Index end = m_matrix.outerIndexPtr()[m_outerStart + m_outerSize.value()]; // ending posiiton of the current block
    Index block_size =
            end - start;                                                // available room in the current block
    ...
    if (nnz > free_size) {
        ...
        std::memmove(matrix.data().valuePtr() + start + nnz, matrix.data().valuePtr() + end,
...
SparseMatrixBase<Derived>::innerVector(Index outer) { return InnerVectorReturnType(derived(), outer); }
...
SparseMatrixBase<Derived>::innerVector(Index outer) const { return ConstInnerVectorReturnType(derived(), outer); }
