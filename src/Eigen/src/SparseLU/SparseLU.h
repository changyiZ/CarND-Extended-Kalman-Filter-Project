...
/** \ingroup SparseLU_Module
  ...
  */
    ...
    public:
        SparseLU() : m_isInitialized(true), m_lastError(""), m_Ustore(0, 0, 0, 0, 0, 0), m_symmetricmode(false),
                     m_diagpivotthresh(1.0), m_detPermR(1) {
            initperfvalues();
        }

        SparseLU(const MatrixType &matrix) : m_isInitialized(true), m_lastError(""), m_Ustore(0, 0, 0, 0, 0, 0),
                                             m_symmetricmode(false), m_diagpivotthresh(1.0), m_detPermR(1) {
            initperfvalues();
            compute(matrix);
        }
...
/** 
  ...
  */
    ...
    void SparseLU<MatrixType, OrderingType>::factorize(const MatrixType &matrix) {
        ...
        new(&m_Ustore) MappedSparseMatrix<Scalar, ColMajor, Index>(m, n, m_nnzU, m_glu.xusub.data(), m_glu.usub.data(),
                                                                   m_glu.ucol.data());
        ...
        void solveInPlace(MatrixBase <Dest> &X) const {
            ...
            for (Index k = m_mapL.nsuper(); k >= 0; k--) {
                ...
                if (nsupc == 1) {
                    ...
                    Map<Matrix<Scalar, Dynamic, Dynamic, ColMajor>, 0, OuterStride<> > U(&(X(fsupc, 0)), nsupc, nrhs,
                                                                                         OuterStride<>(n));
